...
def signin(): # Function to handle sign-in process
    ...
    if username == "admin" and password == '283165': # Check if username and password match
        screen = Toplevel(wn) # Create new window for successful sign-in
        screen.title("App")
...
Label(wn,image=img,bg="white").place(x=50 , y=50)

frame = Frame(wn,width=360,height=350,bg="#fff")
